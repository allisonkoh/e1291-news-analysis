...
results
pred_class_desc = model %>% predict_classes(xtest)
pred_class_desc = factor(pred_class_desc)
pred_class_tab_desc = table(actual_class, factor(pred_class_desc, labels = id_order))

result_rnncnn_desc = confusionMatrix(pred_class_tab_desc,mode="everything")
result_rnncnn_desc = data.frame(t(result_rnncnn_desc$byClass))
result_rnncnn_desc$Macro = base::rowMeans(result_rnncnn_desc)